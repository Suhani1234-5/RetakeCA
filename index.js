...
app.get("/signup",(req,res)=>{
    ...
    if (!password){
        return res.status(400).send({error:"Password cannot be empty and it must be minimum 8 characters and cannot exceed 16 characters"})
    }
    if(password.length<8 || password.length>16){
        if(password.length<8){
            return res.status(400).send({error:"Password must be of minimum 8 characters."});
        }
